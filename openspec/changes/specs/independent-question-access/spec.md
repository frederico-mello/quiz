...
### Requirement: Remove sequential flow

The system SHALL remove the sequential quiz flow, question shuffling, and cumulative score statistics.

#### Scenario: No score displayed
- **WHEN** a question is loaded via `?q=<id>`
- **THEN** the system SHALL NOT display "Pontuação média" or any score

#### Scenario: No progress indicator
- **WHEN** a question is loaded via `?q=<id>`
- **THEN** the system SHALL NOT display "Pergunta X de Y" progress text

### Requirement: Fix initialization error

The system SHALL NOT crash on startup when no questions have been answered.

#### Scenario: App loads without error
- **WHEN** the app starts and loads a question via `?q=<id>`
- **THEN** the system SHALL display the question without any ZeroDivisionError or crash
